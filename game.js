(function () {
    // Constants
    // =========
    var PACMAN_SPEED = 5, PACMAN_RADIUS = 0.25;
    ...
    var createWall = function () {
        ...
        var wallMaterial = new THREE.MeshLambertMaterial({ color: 0x95ff89 });
    ...
    var createPacman = function () {
        ...
        var pacmanMaterial = new THREE.MeshPhongMaterial({ color: 0xffc889 , side: THREE.DoubleSide });
    ...
    var main = function () {
        ...
        var movePacman = function (delta) {
            ...
            if (keys['A']) {
                // A - rotate left
                pacman.direction.applyAxisAngle(UP, Math.PI / 2 * delta + 0.01);
            }
            if (keys['D']) {
                // D - rotate right
                pacman.direction.applyAxisAngle(UP, -Math.PI / 2 * delta - 0.01);
